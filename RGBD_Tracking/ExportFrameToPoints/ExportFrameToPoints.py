...
class ExportFrameToPointsWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
  ...
  def onApplyButton(self):
    minThreshold = self.ui.thresholdSlider.minimumValue
    maxThreshold = self.ui.thresholdSlider.maximumValue
  ...
  def convertDepthToPoints(self):
    ...
    for x in range(0,imageShape[0],10):
      for y in range(0,imageShape[1],10):
        ...
        if depthValue >= min_depth + max_depth*(self.minThreshold/100.0) and depthValue <= max_depth*((self.maxThreshold/100.0)):
